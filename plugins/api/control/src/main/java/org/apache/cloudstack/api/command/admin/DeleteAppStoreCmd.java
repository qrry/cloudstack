...
@APICommand(name = "deleteAppStore",
        description = "应用商店：删除应用",
        since = "4.15.0",
        responseObject = AppStoreResponse.class,
        requestHasSensitiveInfo = false,
        responseHasSensitiveInfo = false)
public class DeleteAppStoreCmd extends BaseCmd {
    public static final Logger s_logger = Logger.getLogger(DeleteAppStoreCmd.class.getName());

    private static final String s_name = "deleteAppStoreResponse";
    ...
    @Override
    public void execute() {
        boolean result = _apiControlAppStoreService.deleteAppStore(this);
        if (result) {
            SuccessResponse response = new SuccessResponse(getCommandName());
            this.setResponseObject(response);
        } else {
            throw new ServerApiException(ApiErrorCode.INTERNAL_ERROR, "应用商店：删除应用失败");
        }
    }
}
