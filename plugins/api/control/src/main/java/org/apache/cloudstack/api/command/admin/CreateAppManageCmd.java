...
@APICommand(name = "createAppManage",
        description = "应用管理：新增应用",
        since = "4.15.0",
        responseObject = AppManageResponse.class,
        requestHasSensitiveInfo = false,
        responseHasSensitiveInfo = false)
public class CreateAppManageCmd extends BaseCmd {
    public static final Logger s_logger = Logger.getLogger(CreateAppManageCmd.class.getName());

    private static final String s_name = "createAppManageResponse";
    ...
    @Parameter(name = ControlConstants.AppManage.IP, type = CommandType.STRING, required = true, description = "应用管理实例维护IP")
    private String ip;

    @Parameter(name = ControlConstants.AppManage.PORT, type = CommandType.STRING, required = true, description = "应用管理实例维护端口")
    private String port;
    ...
    @Override
    public void execute() {
        ControlAppManageVO result = _apiControlAppManageService.createAppManage(getAppStoreId(), getDescription(),
                getInstanceId(), getInstanceName(), getRunScript(), getIp(), getPort(), getLoginUser(),
                getLoginPassword(), getState(), getRemark());
        if (result != null) {
            ...
            throw new ServerApiException(ApiErrorCode.INTERNAL_ERROR, "应用管理：创建应用失败");
        }
    }
}
